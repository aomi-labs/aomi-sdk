//! The operation model the tool layer renders from, and how it is read out of
//! a parsed OpenAPI spec.
//!
//! `mod.rs` drives generation; everything here answers "what operations does
//! this spec expose, and what shape are their params and responses?".
...
pub(crate) fn map_param(p: &openapiv3::Parameter) -> Param {
...
pub(crate) fn schema_kind(format: &openapiv3::ParameterSchemaOrContent) -> ParamKind {
    use openapiv3::{
...
pub(crate) fn path_param_order(path: &str) -> Vec<&str> {
    let mut out = Vec::new();
...
pub(crate) fn derive_response_summary(
...
pub(crate) fn first_success_content_type(op: &openapiv3::Operation) -> Option<String> {
    use openapiv3::ReferenceOr;
    for (code, resp_ref) in &op.responses.responses {
        let is_2xx = matches!(code, openapiv3::StatusCode::Code(c) if (200..300).contains(c))
            || matches!(code, openapiv3::StatusCode::Range(2));
...
pub(crate) fn header_looks_like_auth(name: &str) -> bool {
    let n: String = name
        .chars()
