...
use op::{Op, ParamKind, collect_ops};
...
pub fn run(args: GenToolArgs) -> Result<()> {
    ...
    // Detect mode ("app-local": client in apps/<platform>/src/client/, tool
    // layer imports `crate::client`; "shared": client in ext/src/<platform>/,
    // tool layer imports `aomi_ext::<platform>`): explicit --shared OR
    ...
    let app_client_dir = root
        ...
        .join("src")
        .join("client");
    let mode = if !args.shared && app_client_dir.exists() {
        "app-local"
    } else {
        "shared"
    };
    println!(
        "Mode: {}",
        if mode == "app-local" {
            "app-local (client in apps/<platform>/src/client/)"
        } else {
            "shared (client in ext/src/<platform>/)"
        }
    );
