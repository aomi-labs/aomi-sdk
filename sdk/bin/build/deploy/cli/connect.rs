...
use crate::deploy::backend::{BackendClient, oauth_start};
use crate::deploy::config::AomiConfig;
use crate::deploy::flow::{TokenCheck, validate_activation_token};
...
impl ConnectArgs {
    pub async fn run(self) -> Result<()> {
        ...
        let start =
            oauth_start(&backend_url, self.platform.as_str(), repo.as_deref(), mode).await?;
        ...
        let claimed = match (repo.as_ref(), build_url.as_deref()) {
            (Some(repo), Some(build_url)) => {
                let builder = login::ensure_logged_in(build_url, self.no_browser).await?;
                let github_user_id =
