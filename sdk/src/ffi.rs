...
/// inherits the canonical namespace composition):
/// ```rust,ignore
/// use aomi_sdk::AppVariant;
/// dyn_aomi_app!(app = MarinadeApp, name = "marinade", version = "0.1.0",
///     preamble = "...", tools = [...],
///     variant = AppVariant::SvmSelfBroadcast);
/// ```
///
/// **Variant + extra namespaces** (variant defaults are unioned with the
/// explicit list at the host loader):
/// ```rust,ignore
/// dyn_aomi_app!(app = HybridApp, name = "hybrid", version = "0.1.0",
///     preamble = "...", tools = [...],
///     variant = AppVariant::SvmAppBroadcast,
///     namespaces = ["database"]);
/// ```
#[macro_export]
macro_rules! dyn_aomi_app {
    // ── With variant + secrets + namespaces ──────────────────────────────
    (
        app = $app_type:ty,
        name = $name:expr,
        ...
        impl $crate::DynAomiApp for $app_type {
            ...
            fn variant(&self) -> ::std::option::Option<$crate::AppVariant> {
    ...
    // ── With variant + namespaces (no secrets) ───────────────────────────
    (
        app = $app_type:ty,
        name = $name:expr,
        version = $version:expr,
    ...
    // ── With variant + secrets (no extra namespaces) ─────────────────────
    (
        app = $app_type:ty,
        name = $name:expr,
        version = $version:expr,
        ...
        impl $crate::DynAomiApp for $app_type {
            ...
            // No explicit `namespaces()` — variant.default_namespaces()
            // is the source of truth (host loader composes it).
            fn namespaces(&self) -> ::std::option::Option<::std::vec::Vec<::std::string::String>> {
