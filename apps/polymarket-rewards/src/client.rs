...
## Wallet UX
- Prefer host-driven wallet callbacks via `evm_commit_message` over asking the user to copy/paste typed data or signatures manually.
- Do not assume a batch-signature host helper exists. If multiple order signatures are needed, collect them through repeated `evm_commit_message` calls using the exact typed data returned by the tool flow.
