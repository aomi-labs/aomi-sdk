        ...
        F: Fn(&reqwest::blocking::Client, &str, &str) -> reqwest::blocking::RequestBuilder,
    {
        ...
        loop {
            ...
            if status == reqwest::StatusCode::TOO_MANY_REQUESTS && attempts < MAX_RETRIES {
                attempts += 1;
                eprintln!(
                    "[para] {op} rate limited (429), retry {attempts}/{MAX_RETRIES} in {backoff_ms}ms"
                );
            ...
            if !status.is_success() {
                let hint = match status.as_u16() {
                    401 | 403 => {
                        " Hint: Check that the Para API key is valid and IP is allowlisted."
                    }
                ...
                return Err(format!(
                    "[para] {op} failed with HTTP {status}: {body}{hint}"
                ));
            }
